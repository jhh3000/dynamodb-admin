...
const { exportTable } = require('./actions/exportTable')
...
exports.createServer = (dynamodb, docClient) => {
  ...
  app.get('/tables/:TableName/export', asyncMiddleware((req, res) => {
    return exportTable(req.params.TableName, dynamodb)
      .then((data) => {
        res.set('Content-disposition', `attachment; filename=${req.params.TableName}.csv`);
